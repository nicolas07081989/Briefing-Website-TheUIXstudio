    ...
    function tuixb_shortcode() {
        ...
        <div class="tuixb-wrap" data-tuixb="1" data-ajax="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce('tuixb_nonce')); ?>">
            <form class="tuixb-form" novalidate enctype="multipart/form-data">
                ...
                <div class="tuixb-progress"><div class="tuixb-progress-bar"></div></div>
                ...
                <div class="tuixb-steps">
                    ...
                    <section class="tuixb-step" data-step="999" data-summary-step="1">
                        <h3>Revisión final</h3>
                        <div class="tuixb-summary"></div>
                    </section>
                </div>

                <div class="tuixb-footer">
                    <button type="button" class="tuixb-btn tuixb-btn-ghost" data-tuixb-prev="1">Anterior</button>
                    <button type="button" class="tuixb-btn tuixb-btn-main" data-tuixb-next="1">Siguiente</button>
                    <button type="submit" class="tuixb-btn tuixb-btn-main tuixb-submit">Enviar briefing</button>
                </div>

                <div class="tuixb-message" role="status" aria-live="polite"></div>
        ...
        (function(){
            function initTuixb(root){
                ...
                const prevBtn = root.querySelector('[data-tuixb-prev="1"]');
                const nextBtn = root.querySelector('[data-tuixb-next="1"]');
                ...
                const validateStep = () => {
                    ...
                    if(!current || current.hasAttribute('data-summary-step')) return true;
                    clearErrors(current);
                    let ok = true;
                ...
                const goTo = (idx) => {
                    ...
                    progress.style.width = (((active + 1) / vis.length) * 100) + '%';
                    prevBtn.style.visibility = active === 0 ? 'hidden' : 'visible';
                    const inSummary = vis[active].hasAttribute('data-summary-step');
                ...
                const postForm = async () => {
                    ...
                    let json;
                    try { json = JSON.parse(raw); } catch(_e) { throw new Error('Respuesta no válida del servidor.'); }
                    if(!json || !json.success){
                        throw new Error((json && json.data && json.data.message) ? json.data.message : 'No se pudo enviar el briefing.');
                    }
                    return json;
                };
                ...
                setPlan(planInput.value || '30');
            }

            const boot = () => document.querySelectorAll('.tuixb-wrap[data-tuixb="1"]').forEach(initTuixb);
            if(document.readyState === 'loading'){
                document.addEventListener('DOMContentLoaded', boot);
            } else {
                boot();
            }
            window.addEventListener('elementor/popup/show', boot);
